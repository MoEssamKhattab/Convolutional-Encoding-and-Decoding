...
def bpsk_modulator(bit_seq, fc, Ab, Tb, n):
    """
    BPSK modulator
    ...
    for i in range(num_bits):
        ...
        passbannd_signal[i*n:(i+1)*n] = basis_function

def generate_message(Eb):
    """
    Generate message signal for BPSK modulation in signal space
    :param Eb: energy per bit
    :return: message signal
    """
    return np.sqrt(Eb), -np.sqrt(Eb)   # s11 -> symbol 0, s21 -> symbol 0
